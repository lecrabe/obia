...
try:
    ...
    rasterize = "gdal_rasterize -burn 1 -l %s -ot UInt32 -te %r %r %r %r -tr %r %r -co \"COMPRESS=LZW\" %s %s/tmp_mask.tif\n" %   (layer,im_xmin,im_ymin,im_xmax,im_ymax,pxsz,pxsz,vector,tmpdir)
...
################# Compress and output
compress = "gdal_translate -ot UInt32 -co \"COMPRESS=LZW\" %s/tmp_mask.tif %s" % (tmpdir,outfile) 
print("-"*40 + "\n" + compress)
os.system(compress)
